...
class ColorOrder:
    ...
    # Сортировка
    def sort(self):
        # Я бы использовал такую сортировку, так как этот вариант будет читабельнее, короче и легче, что важно,
        # но сортировка слиянием в данном алгоритме будет гораздо быстрее, да и проверки на неправильные данные можно
        # внедрить прям внутрь процессов сортировки(что здесь и сделано)
        ...
        # Слияние. Вообще можно было сделать рекурсивно, но я предпочел данный вариант
        output = []
        if self.order[0] == "с":
